...
def arg_parse():
    ...
    parser.add_argument(
        ...
        default=r"G:\data\rssrai2019_scene_classification",
    )
    parser.add_argument(
...
def net_test(net, data, device, epoch, classes_label):
    ...
    for data in testloader:
        ...
        _, predicted = torch.max(outputs.data, 1)
        print(torch.nn.functional.softmax(outputs, 1))
        total += labels.size(0)
...
def main():
    ...
    dataname = "rssrai2019"
    model_name = "ResNet50"
    data = Dataset.Sencedata(datadir, BATCH_SIZE)
    traindata, testdata = data.getdata()
    classes_label = data.label()
    net = torchvision.models.resnet18()
    resnet18_pretrained_model = r'D:\data\.torch\models\resnet18-5c106cde.pth'
    pretrained_model = resnet18_pretrained_model
    net.load_state_dict(torch.load(pretrained_model))
    net.fc = nn.Linear(net.fc.in_features, len(classes_label))
    # net.load_state_dict(torch.load(pretrained_model))
    # fc training or all param training
